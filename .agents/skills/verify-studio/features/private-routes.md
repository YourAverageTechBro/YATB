...
- `origin-rejection` rejects a Better Auth write from another origin.
...
- **Request media.** Request `/api/media/missing` without cookies. The response is `401` with private no-store caching.
- **Revoke.** Set the signed-in email's `allowed_email.enabled` value to `0`, then reload `/videos`. The browser returns to `/`.
- **Reject origin.** Submit an auth write with `Origin: https://attacker.example`. The response is `403` with `INVALID_ORIGIN`.
