...
import { addComment, loadComments, loadDrafts, removeComment, saveComment } from '#/server/reviews.functions'
...
export function ReviewWorkspace({ videoId, onFootageChanged }: { videoId: string; onFootageChanged: () => void }) {
  ...
  return <section className="review-workspace" aria-labelledby="review-heading">
    ...
    {drafts.length === 0 && uploads.length === 0
      ...
      : <>
        {selected && <div className="draft-selector"><Label>Draft version<NativeSelect value={selected.id} onChange={(event) => setSelectedId(event.target.value)}>{drafts.map((draft) => <NativeSelectOption key={draft.id} value={draft.id}>Version {draft.version} · {draft.file.displayName}</NativeSelectOption>)}</NativeSelect></Label><small>{drafts.length} immutable {drafts.length === 1 ? 'version' : 'versions'} · newest first</small></div>}
        {selected && <ReviewPane key={selected.id} draft={selected} onFootageChanged={onFootageChanged} />}
      </>}
  </section>
}

function ReviewPane({ draft, onFootageChanged }: { draft: Draft; onFootageChanged: () => void }) {
  ...
  return <div className="review-pane">
    ...
    <Card className="comment-composer">
      <form onSubmit={(event) => void submit(event)}>
        ...
        <Label className={buttonVariants({ variant: 'outline' })}>Attach images or video<Input className="sr-only" type="file" accept="image/*,video/*" multiple onChange={(event) => { setAttachments([...(event.target.files ?? [])].slice(0, 12).map((file) => ({ id: crypto.randomUUID(), file }))); event.target.value = '' }} /></Label>
